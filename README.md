...
首次发布前，请在仓库 **Settings → Pages** 将发布源设为 **GitHub Actions**。推送形如 `v0.6.0` 的 tag 后，发布工作流会创建 Release、上传 VSIX 与 `checksums.txt` 和升级元数据；后续 Pages 工作流会在默认分支上下文部署 `update/stable.json`。因此 `github-pages` Environment 的分支保护可以继续只允许默认分支，无需向发布 tag 放行。
