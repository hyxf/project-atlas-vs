...
标题栏按钮从左到右依次为新增、编辑收藏（Edit npm Favorites）、刷新。命令面板提供 `Project Atlas: Refresh npm Packages`、`Search npm Packages`、`Add npm Package` 和 `Edit npm Favorites`。移除、恢复、永久删除依赖、打开包主页和单个包的收藏操作需从树节点菜单触发。
